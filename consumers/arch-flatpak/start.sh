...
# 1. Start system D-Bus (needed by flatpak)
mkdir -p /run/dbus
dbus-daemon --system --fork

# 2. Launch everything inside a session bus via dbus-run-session.
...
exec su -s /bin/bash consumer -c '
    ...
    exec dbus-run-session -- bash -c "
        # 3. Start gnome-keyring-daemon with an unlocked login keyring.
        ...
        # 4. Start xdg-desktop-portal (discovers gnome-keyring for Secret portal)
        /usr/lib/xdg-desktop-portal &
        sleep 2

        # 5. Run the trigger server
